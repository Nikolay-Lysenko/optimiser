...
from optimiser.utils import map_in_parallel
...
def report_current_results(
    ...
    :param population:
        number of a current population
    :param top_entries:
        top entries for a current population
    :param best_entry:
        the best entry amongst all populations
    :return:
        None
    """
    top_scores = [x['score'] for x in top_entries]
    avg_top_score = np.mean(top_scores)
    res = f"{population:>10} | {avg_top_score:>26} | {best_entry['score']:>19}"
    print(res)


def optimize_with_crossentropy_method(
        target_fn: Callable[[np.ndarray], float],
        n_populations: int,
        population_size: int,
    ...
    :param parallelling_params:
        ...
        tasks are finished
    :param verbose:
        if it is set to `True`, current progress is printed
    ...
    for i_population in range(n_populations):
        ...
        if new_best_entry['score'] > best_entry['score']:
            best_entry = new_best_entry

        if verbose:
            report_current_results(i_population, top_entries, best_entry)
    found_params = best_entry['params']
    return found_params
